...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Create the 25 paper EEG2Mel train/validation/test splits."
    )
    parser.add_argument(
        "--config",
...
if __name__ == "__main__":
    ...
    split_output_dir = base_dir / "runs" / run_name / "splits"
    ...
    if loso_fname is not None:
        loso_path = base_dir / "runs" / run_name / "splits" / loso_fname

    song_path = None
    if song_fname is not None:
        song_path = base_dir / "runs" / run_name / "splits" / song_fname

    subject_song_path = None
    if subject_song_fname is not None:
        subject_song_path = (
            base_dir / "runs" / run_name / "splits" / subject_song_fname
        )
    ...
    if random_segment_out_fname is not None:
        create_random_segment_out_split(
            ...
            n_folds=random_segment_out_config.get("n_folds", 1),
        )
    ...
    if loso_fname is not None:
        create_subject_out(
            ...
            n_folds=subject_randomize_config.get("n_folds", 1),
        )
    ...
    if subject_song_fname is not None:
        ...
        create_subject_song_out(
            ...
            n_folds=combined_randomize_config.get("n_folds", 1),
        )
